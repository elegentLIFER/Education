...
//				mAnchor.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION);
    ...
    private void initControllerView(View v) {
        ...
        ivFullScreen = (ImageView) v.findViewById(getResources().getIdentifier("iv_full_screen", "id", mContext.getPackageName()));
        ivFullScreen.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                onControllerClick.OnFullScreenClick();
            }
        });
        tvSpeed = (TextView) v.findViewById(getResources().getIdentifier("tv_speed", "id", mContext.getPackageName()));
    ...
    public void showSpeedAjust(boolean show) {
        if (tvSpeed!=null)
        tvSpeed.setVisibility(show ? VISIBLE : GONE);
    }

    String[] strings = {"0.8", "1.0", "1.25", "1.5", "2.0"};

    // TODO: 2018/10/25 0025 我的弹窗
    private void showFixedTypeDialog() {
        if (speedPopWindow == null) {
            speedPopWindow = new ListPopupWindow(getContext());
            ArrayAdapter<String> fixedGameTypeListAdapter = new ArrayAdapter<>(getContext(), android.R.layout.simple_list_item_1, strings);
            speedPopWindow.setAdapter(fixedGameTypeListAdapter);
            speedPopWindow.setAnchorView(container);
//            speedPopWindow.setDropDownGravity(Gravity.BOTTOM);
            speedPopWindow.setWidth(container.getWidth());
            speedPopWindow.setHeight(container.getHeight());
            speedPopWindow.setModal(true);
            speedPopWindow.setVerticalOffset(10);
            speedPopWindow.setOnItemClickListener(this);
        }
        speedPopWindow.show();
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        Float speed = Float.valueOf(strings[position]);
        onControllerClick.OnSpeedSelect(speed);
        if (speedPopWindow.isShowing()) speedPopWindow.dismiss();
    }
    ...
    public void setFileName(String name) {
        mTitle = name;
        android.util.Log.e(TAG, "setFileName: " + name);
        if (mFileName != null)
            mFileName.setText(mTitle);
    }
